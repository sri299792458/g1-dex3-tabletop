...
def build_parser() -> argparse.ArgumentParser:
    ...
    inspect_plan = subparsers.add_parser(
        "inspect-plan", help="validate a frozen bilateral plan and print its summary"
    )
    ...
    bilateral_collect.add_argument("--maximum-capture-attempts", type=int, default=1)
...
def run_plan_bilateral_calibration(args: argparse.Namespace) -> int:
    ...
    from g1_dex3_tabletop.calibration.hand_posture import check_full_close

    check_full_close(
        left_q_rad=snapshot.left_dex3_q_rad,
        right_q_rad=snapshot.right_dex3_q_rad,
        tolerance_rad=0.08,
    )
    dex3_command_positions = {
        side: getattr(snapshot, f"{side}_dex3_q_rad") for side in ("left", "right")
    }
    dex3_model_positions = dict(dex3_command_positions)
...
def run_inspect_plan(args: argparse.Namespace) -> int:
    plan = BilateralExecutionPlan.from_json(args.plan)
    durations = [item.trajectory.sample_time_s[-1] for item in plan.transitions]
    print(
        json.dumps(
            {
                "commands_robot": False,
                "plan_sha256": plan.content_sha256,
                "pose_design_sha256": plan.pose_design_sha256,
                "trajectory_count": len(plan.transitions),
                "route_duration_s": sum(durations),
                "returns_to_handoff": plan.transitions[-1].trajectory.to_pose_id == "__handoff__",
                "minimum_clearance_m": plan.self_clearance_certificate["minimum_clearance_m"],
                "required_clearance_m": plan.self_clearance_certificate["hard_clearance_m"],
