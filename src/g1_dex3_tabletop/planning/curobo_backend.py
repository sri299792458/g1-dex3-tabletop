...
def plan_dex3_preparation(
    ...
    *,
    ...
    commissioned_shoulders: bool = False,
) -> Dex3PreparationPlan:
    """Plan the commissioned right-then-left shoulder clearance with CuRobo."""

    if not request.holds_measured_fingers:
        validate_dex3_finger_targets(
            left_q_rad=request.left_target_q_rad,
            right_q_rad=request.right_target_q_rad,
            label="calibration close target",
        )
    measured_restoration = (
        request.left_return_target_q_rad == request.snapshot.left_dex3_q_rad
        and request.right_return_target_q_rad == request.snapshot.right_dex3_q_rad
    )
    if request.left_return_target_q_rad is not None and not measured_restoration:
        validate_dex3_finger_targets(
    ...
    maximum_offset = request.maximum_outward_offset_rad
    offsets = np.arange(
        request.initial_outward_offset_rad,
        maximum_offset + 1e-12,
        request.outward_search_step_rad,
    )
    offsets = np.minimum(offsets, maximum_offset)
    ...
    for index, offset in enumerate(offsets, start=1):
        ...
        try:
            ...
            sweep_count = (
                0
                if request.holds_measured_fingers
                else _validate_curobo_finger_sweep(request=request, snapshot=after_dual)
            )
            ...
            if request.left_return_target_q_rad is not None:
                ...
                if measured_restoration:
                    return_request = Dex3PreparationRequest.for_measured_restoration(
                        snapshot=return_snapshot,
                        recorded_start=request.snapshot,
                        joint_position_offsets_rad=request.joint_position_offsets_rad,
                        left_target_q_rad=request.left_return_target_q_rad,
                        right_target_q_rad=request.right_return_target_q_rad,
                        random_seed=request.random_seed,
                ...
                if not request.holds_measured_fingers:
                    return_sweep_count = _validate_curobo_finger_sweep(
                        ...
                        snapshot=return_request.snapshot,
                    )
        ...
        return Dex3PreparationPlan(
            ...
            planner_provenance={
                **model_source_hashes(),
                "route_policy": (
                    "right_shoulder_then_left_shoulder_with_measured_finger_hold"
                    if request.holds_measured_fingers
                    else "right_shoulder_then_left_shoulder_then_close_sweep"
                    + ("_and_return_sweep" if return_sweep_count else "")
                ),
                "finger_motion_policy": (
                    "operator_preclosed_measured_hold"
                    if request.holds_measured_fingers
                    else "command_targets"
                ),
                "execution_maximum_velocity_rad_s": EXECUTION_MAXIMUM_VELOCITY_RAD_S,
                "commissioned_straight_shoulders": commissioned_shoulders,
                ...
                "return_finger_sweep_direction": (
                    "none"
                    if request.holds_measured_fingers
                    else "reverse_measured_restoration"
                    if measured_restoration
                    else "forward"
                ),
    ...
    raise RuntimeError(
        "No collision-clear shoulder/finger preparation was found within "
        f"{request.initial_outward_offset_rad:.2f}–{maximum_offset:.2f} rad "
        f"({len(offsets)} candidates). Stopped before motion. "
        "Adjust the hand position and rerun preflight. " + detail
    )
...
def _plan_clearance_arm(
    *,
    ...
    if source_collision:
        ...
        trajectory, diagnostic = _strict_linear_joint_trajectory(
            robot=robot,
            device_cfg=device_cfg,
            source_q=reference,
            target_q=target,
...
def certify_loaded_shoulder_return(
    *, snapshot, preparation, joint_position_offsets_rad, device_cfg=None
):
    """Check the original reverse shoulder route against today's loaded body."""
    if device_cfg is None:
        import torch
        from curobo.types import DeviceCfg

        device_cfg = DeviceCfg(device=torch.device("cuda:0"), dtype=torch.float32)
    ready = np.asarray(snapshot.measured_q29_rad).copy()
    ready[15:22] = preparation.left_return.command_q_rad[-1]
    ready[22:29] = preparation.right_return.command_q_rad[-1]
    dual = ready.copy()
    dual[15:29] = preparation.dual_clearance_q14_rad
    right = dual.copy()
    right[15:22] = ready[15:22]
    certificate = _certify_bilateral_arm_segments(
        snapshot=snapshot,
        reference_q29=ready,
        joint_position_offsets_rad=joint_position_offsets_rad,
        segments=(
            ("left_shoulder_return", "left", preparation.left_return, dual),
            ("right_shoulder_return", "right", preparation.right_return, right),
        ),
        device_cfg=device_cfg,
        phase="return_hand_shoulder_preparation",
    )
    if not certificate["passed"]:
        raise ValueError(
            "commissioned shoulder return failed loaded-body validation: "
            f"{certificate['minimum_margin_link_pair']}, "
            f"margin {certificate['minimum_margin_to_required_clearance_m'] * 1000:.2f}mm; "
            "the recorded Ready return cannot be certified under this body posture"
        )
    return certificate
...
def plan_bilateral_calibration_adapter(
    ...
    *,
    progress: Callable[[str], None] | None = None,
    preparation: Dex3PreparationPlan | None = None,
) -> BilateralCalibrationAdapterPlan:
    ...
    preparation_request = Dex3PreparationRequest.for_measured_hold(
        snapshot=request.snapshot,
        joint_position_offsets_rad=request.joint_position_offsets_rad,
        random_seed=request.random_seed,
    )
    already_at_clearance = preparation is not None
    if preparation is None:
        report("planning commissioned straight Ready-to-shoulder-clearance preparation")
        preparation = plan_dex3_preparation(
            preparation_request, progress=report, commissioned_shoulders=True
        )
    elif not np.allclose(
        live_q29[arm_index_array], preparation.dual_clearance_q14_rad, atol=1e-9, rtol=0
    ):
        raise ValueError("loaded adapter must begin at the completed shoulder command")
    if preparation.planner_provenance.get("finger_motion_policy") != (
        "operator_preclosed_measured_hold"
    ):
        raise ValueError("calibration adapter requires a measured-finger-hold shoulder plan")
    ...
    clearance_snapshot = RobotSnapshot(
        ...
        left_dex3_q_rad=request.snapshot.left_dex3_q_rad,
        right_dex3_q_rad=request.snapshot.right_dex3_q_rad,
    )
    device_cfg = DeviceCfg(device=torch.device("cuda:0"), dtype=torch.float32)
    if already_at_clearance:
        report("validating the commissioned shoulder return under the loaded body")
        return_certificate = certify_loaded_shoulder_return(
            snapshot=request.snapshot,
        ...
        return_certificate = preparation.planner_provenance[
            "return_arm_self_clearance_certificate"
        ]
    ...
    return BilateralCalibrationAdapterPlan(
        ...
        planner_provenance={
            **model_source_hashes(),
            ...
            "policy": "commissioned_shoulders_then_loaded_anchor_core"
            if already_at_clearance
            else "read_only_commissioned_shoulders_anchor_core",
            "loaded_shoulder_return_certificate": return_certificate,
            "locked_body_policy": "measured_live_body_and_fingers_with_full_core_clearance_replay",
            "finger_motion_policy": "operator_preclosed_measured_hold",
            "held_dex3_joint_positions_rad": {
                "left": list(request.snapshot.left_dex3_q_rad),
                "right": list(request.snapshot.right_dex3_q_rad),
            },
        },
    )
    ...
    *,
    ...
    try:
        ...
        return BilateralRoutePlanningResult(
            ...
            planner_provenance={
                "finger_motion_policy": request.dex3_preparation_plan.planner_provenance.get(
                    "finger_motion_policy", "command_targets"
                ),
                **model_source_hashes(),
                "device": torch.cuda.get_device_name(device_cfg.device),
                "route_elapsed_s": time.monotonic() - planning_started,
