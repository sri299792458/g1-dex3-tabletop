...
class BilateralCalibrationPlanningRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or (
                not preparation_request.holds_measured_fingers
                and preparation_plan.return_sweep_sample_count < 2
            )
            or (
                preparation_request.holds_measured_fingers
                and (
                    preparation_plan.planner_provenance.get("finger_motion_policy")
                    != "operator_preclosed_measured_hold"
                    or preparation_plan.finger_sweep_sample_count != 0
                    or preparation_plan.return_sweep_sample_count != 0
                )
            )
        ):
            raise ValueError(
...
class BilateralRoutePlanningResult:
    ...
    def __post_init__(self) -> None:
        ...
        measured_hold = self.planner_provenance.get("finger_motion_policy") == (
            "operator_preclosed_measured_hold"
        )
        if measured_hold and (
            self.finger_sweep_sample_count or self.restoration_sweep_sample_count
        ):
            raise ValueError("measured-hold route cannot contain finger sweeps")
        if transitions and not measured_hold and self.finger_sweep_sample_count < 2:
            raise ValueError("connected bilateral route requires a certified closing sweep")
        if transitions and not measured_hold and self.restoration_sweep_sample_count < 2:
            raise ValueError(
    ...
    def validate_request(self, request: BilateralRoutePlanningRequest) -> None:
        ...
        if (
            self.connected
            and request.dex3_preparation_request.holds_measured_fingers
            and (
                self.planner_provenance.get("finger_motion_policy")
                != ("operator_preclosed_measured_hold")
                or self.finger_sweep_sample_count
                or self.restoration_sweep_sample_count
            )
        ):
            raise ValueError("measured-hold route result differs from its finger policy")
        rejectable_ids = {
...
def _select_bilateral_anchor(
    ...
    *,
    ...
    sightline_rejections = []
    for choice in sorted(candidates, key=lambda item: (item[0], item[1], item[2], item[3])):
        clearance_motion, negative_separation, left_id, right_id, anchor_q = choice
        if sightline_checker is not None:
            try:
                sightline_checker.check(anchor_q, required_sides=_SIDES)
            except ValueError as error:
                sightline_rejections.append(
                    {"candidate_id": f"{left_id}+{right_id}", "reason": str(error)}
                )
                continue
        break
    else:
        raise ValueError(
            "no bilateral anchor has clear marker sightlines; "
            + sightline_rejections[-1]["reason"]
        )
    ...
    return (
        ...
        anchor_q,
        {
            ...
            "sightline_rejections": sightline_rejections,
            "sightline_check": sightline_checker.provenance
            if sightline_checker is not None
            else {},
...
def build_bilateral_design_pool(
    ...
    *,
    ...
    """Require visible active markers and a both-visible fixed anchor."""
    ...
    if sightline_checker is None:
        from g1_dex3_tabletop.calibration.visibility import BilateralSightlineChecker

        sightline_checker = BilateralSightlineChecker(request, urdf_model)
    projection = BilateralCalibrationProjection(
        urdf_model,
    ...
    anchor_sources, anchor_q, anchor_search = _select_bilateral_anchor(
        ...
        sightline_checker=sightline_checker,
    )
    ...
    for pose in ik_result.poses:
        ...
        try:
            ...
            coverage = _visibility_bins(request, projected, active_arm=pose.active_arm)
            sightline_checker.check(paired_q, required_sides=(pose.active_arm,))
        except ValueError as error:
            visibility_rejections.append({"candidate_id": pose.candidate_id, "reason": str(error)})
            continue
...
def _predicted_sample(
    ...
    *,
    ...
    observations = {
        ...
        for side in required_marker_sides(active_arm)
    }
    ...
    projected = {
        side: projection.project_side(sample, side=side, parameters=parameters)
