...
def pose_sets_from_bilateral_plan(
    ...
    design_hash = plan.pose_design_sha256
...
@dataclass(frozen=True, slots=True)
class PreparedBilateralCollection:
    """Validated immutable motion instructions prepared before ownership.

    Artifact validation and whole-core JSON hashing must not run alongside
    the real-time driver. Only these small bindings and frozen instructions
    are consumed during collection.
    """
    ...
    @classmethod
    def from_artifacts(cls, design: BilateralPoseDesignArtifact, plan: BilateralExecutionPlan):
        pose_sets = pose_sets_from_bilateral_plan(design, plan)
        return cls(
            plan.content_sha256,
            plan.pose_design_sha256,
            tuple(design.schedule),
            tuple(plan.transitions),
            MappingProxyType(pose_sets),
        )
...
class BilateralCollectionOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        prepared: PreparedBilateralCollection,
        ...
        maximum_capture_attempts: int = 1,
        ...
        if executor.approved_validation_report_sha256 != prepared.plan_sha256:
            raise ValueError("bilateral executor is not bound to the execution plan")
        self.executor = executor
        self.prepared = prepared
        self.pose_sets = prepared.pose_sets
        self.store = store
    ...
    def run(self) -> BilateralCollectionResult:
        ...
        stopping = False
        return_anchor_occurrence_id: str | None = None
        for waypoint_index, waypoint in enumerate(self.prepared.schedule):
            ...
            if waypoint.capturable and not stopping:
                ...
                if stopping and waypoint.capture_role == "anchor":
                    ...
                    break
            if waypoint_index == len(self.prepared.schedule) - 1:
                break
            transition = self.prepared.transitions[waypoint_index]
    ...
    def _activate_arm(self, side: str) -> None:
        ...
        self.executor.switch_validated_arm_plan(
            ...
            approved_validation_report_sha256=self.prepared.plan_sha256,
    ...
    def _capture_waypoint(
        ...
        *,
        ...
        waypoint = self.prepared.schedule[waypoint_index]
        for attempt_index in range(1, self.maximum_capture_attempts + 1):
            ...
            try:
                frames = self.frame_source.capture_burst(
                    ...
                    active_arm=waypoint.active_arm,
                )
                ...
                required = (
                    "both targets"
                    if waypoint.active_arm is None
                    else f"{waypoint.active_arm} target"
                )
                outcome, reason = "accepted", f"{required} passed the stationary burst"
            try:
