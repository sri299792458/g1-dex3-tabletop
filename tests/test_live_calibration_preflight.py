...
@pytest.mark.parametrize("requested_maximum", [0.14, 0.13])
def test_shoulder_search_stops_at_cap_and_requests_hand_adjustment(
    monkeypatch, fake_device, requested_maximum
):
    request = Dex3PreparationRequest(
        ...
        maximum_outward_offset_rad=requested_maximum,
    )
    request = Dex3PreparationRequest.from_dict(request.to_dict())
    attempted = []

    def blocked(**kwargs):
        attempted.append(abs(kwargs["shoulder_roll_delta"]))
        raise ValueError("hand/hip clearance failed")

    monkeypatch.setattr(backend, "_plan_clearance_arm", blocked)
    with pytest.raises(RuntimeError, match="Adjust the hand position and rerun preflight"):
        backend.plan_dex3_preparation(request, commissioned_shoulders=True)
    expected = [0.08, 0.10, 0.12] + ([0.14] if requested_maximum >= 0.14 else [])
    assert attempted == pytest.approx(expected)
    assert max(attempted) <= min(requested_maximum, 0.14)
...
def test_fixed_finger_limit_failure_is_named_and_precedes_offset_search(monkeypatch, phase):
    ...
    request = Dex3PreparationRequest(
        snapshot=RobotSnapshot((0.0,) * 29, left if phase == "return" else outside, right),
        joint_position_offsets_rad={},
...
def test_preflight_restores_recorded_out_of_limit_fingers_with_reverse_sweep(
    monkeypatch, fake_device
):
    from g1_dex3_tabletop.planning.g1_model import validate_dex3_finger_targets

    monkeypatch.setattr(backend, "validate_dex3_finger_targets", validate_dex3_finger_targets)
    left = (0.0, 0.7, 0.7, -1.0, -1.5, -1.0, -1.5)
    right = (0.0, -0.7, -0.7, 1.0, 1.5, 1.0, 1.5)
    recorded_left = (*left[:6], -1.7745474576950073)
    recorded_right = (*right[:6], 1.7745474576950073)
    initial = RobotSnapshot((0.0,) * 29, recorded_left, recorded_right)
    request = Dex3PreparationRequest(
        snapshot=initial,
        joint_position_offsets_rad={},
        ...
        left_settled_target_q_rad=left,
        right_settled_target_q_rad=right,
        left_return_target_q_rad=recorded_left,
        right_return_target_q_rad=recorded_right,
    )
    monkeypatch.setattr(backend, "build_robot_config_for_active_joints", lambda **kw: ({}, ()))
    monkeypatch.setattr(
        backend,
        ...
        lambda **kwargs: SimpleNamespace(
            ...
            self_collision_pair_penetrations=lambda *args, **kw: [{}],
        ),
    )

    def shoulder(**kwargs):
        snapshot = kwargs["snapshot"]
        q = list(snapshot.measured_q29_rad)
        indices = LEFT_ARM_INDICES if kwargs["arm"] == "left" else RIGHT_ARM_INDICES
        q[indices[1]] += kwargs["shoulder_roll_delta"]
        edge = _edge(
            kwargs["from_pose_id"],
            kwargs["to_pose_id"],
            np.array(snapshot.measured_q29_rad)[list(indices)],
            np.array(q)[list(indices)],
        )
        return edge, RobotSnapshot(tuple(q), recorded_left, recorded_right)

    sweeps = []

    def sweep(*, request, snapshot):
        sweeps.append(request)
        assert snapshot.measured_q29_rad[16] == 0.08
        assert snapshot.measured_q29_rad[23] == -0.08
        assert snapshot.left_dex3_q_rad == recorded_left
        assert snapshot.right_dex3_q_rad == recorded_right
        assert request.left_target_q_rad == left
        assert request.right_target_q_rad == right
        return 15

    monkeypatch.setattr(backend, "_plan_clearance_arm", shoulder)
    monkeypatch.setattr(backend, "_validate_curobo_finger_sweep", sweep)
    monkeypatch.setattr(backend, "_certify_bilateral_arm_segments", lambda **kw: {"passed": True})
    monkeypatch.setattr(backend, "model_source_hashes", dict)
    plan = backend.plan_dex3_preparation(request, commissioned_shoulders=True)
    assert len(sweeps) == 2
    assert plan.return_sweep_sample_count == 15
    assert (
        plan.planner_provenance["return_finger_sweep_direction"] == "reverse_measured_restoration"
    )
...
def test_adapter_uses_live_body_and_requires_core_replay(
    ...
    def preparation(prep_request, **kwargs):
        ...
        return Dex3PreparationPlan(
            ...
            planner_provenance={
                ...
                "return_arm_self_clearance_certificate": {"passed": True},
            },
        )
    ...
    def anchor_edge(**kwargs):
        ...
        assert kwargs["snapshot"].left_dex3_q_rad == request.snapshot.left_dex3_q_rad
