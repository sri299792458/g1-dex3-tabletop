...
from g1_dex3_tabletop.calibration.fingers import command_validated_finger_posture
from g1_dex3_tabletop.hardware_motion import _stage_trajectory


@pytest.mark.parametrize("failure", [None, "worker", "binding", "clearance", "state", "control"])
@pytest.mark.parametrize("restoring", [False, True])
def test_loaded_finger_motion_requires_validated_current_state(
    monkeypatch, tmp_path, failure, restoring
):
    from g1_dex3_tabletop.calibration import fingers as hardware
    ...
    left_target = (0.5,) * 6 + (-1.7745474576950073,) if restoring else (0.5,) * 7
    right_target = (-0.5,) * 7
    recorded_start = RobotSnapshot((0.0,) * 29, left_target, right_target)
    calls = []
    checks = []
    ...
    watchdog = SimpleNamespace(pulse=lambda: None)
    ...
    def validate(operation, *, payload, control_check, timeout_s):
        ...
        assert request.snapshot.left_dex3_q_rad == (
            left_target if restoring else hands.left.position
        )
        assert request.left_target_q_rad == (hands.left.position if restoring else left_target)
        assert request.snapshot.right_dex3_q_rad == (
            right_target if restoring else hands.right.position
        )
        assert request.right_target_q_rad == (hands.right.position if restoring else right_target)
        control_check()
        if failure == "worker":
    ...
    kwargs = {
        ...
        "left_target_q_rad": left_target,
        "right_target_q_rad": right_target,
        ...
        "phase": "loaded_close",
        "label": "close",
        "recorded_restoration_start": recorded_start if restoring else None,
    }
    if failure:
        with pytest.raises(RuntimeError):
            command_validated_finger_posture(**kwargs)
        assert calls == []
    else:
        assert command_validated_finger_posture(**kwargs) is hands
        assert len(calls) == 1 and calls[0]["left_acceptance_q_rad"] == (0.49,) * 7
        assert calls[0]["left_target_q_rad"] == left_target
        assert calls[0]["right_target_q_rad"] == right_target
        motion = json.loads((tmp_path / "loaded_close_motion.json").read_text())
        assert motion["validation_direction"] == (
            "reverse_measured_restoration" if restoring else "forward"
        )
        assert motion["left_target_q_rad"] == list(left_target)
        assert len(checks) >= 3
...
def test_stage_failure_stops_driver_before_returning_to_safety_cleanup(
    ...
    from g1_dex3_tabletop import hardware_motion as hardware
