...
Every accepted frame must contain both hand targets. This is an experimental
collector, separate from the calibration bundle used by the stacking demo.
The deployed bundle and the older single-arm command remain available; this
baseline does not establish a replacement calibration from the pilot.
...
The research guide describes the read-only replay tools, measured results and
limitations in its
[camera state-estimation chapter](https://sri299792458.github.io/g1-research-docs/perception/state-estimation.html).
